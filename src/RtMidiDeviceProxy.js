...
const debugCalls = !!process.argv.find(a => a == '--debug-midi-calls')
class RtMidiDeviceProxy {
    ...
    displayName() {
        return `Rtm${this.isOutput ? 'Out' : 'In'} ${this.deviceNumber}: ${this.name || 'Not_Yet_Named'}`
    }
    proxyMethod(name) {
        this[name] = function() {
            const args = JSON.stringify(arguments)
            console.log(`\nRtMidi: calling ${name} on (${this.displayName()}) with ${args}`)
            const result = this.realDevice[name].apply(this.realDevice, arguments)
            console.log(`RtMidi: returning ${result} for ${name} on ${this.tag} with ${args}`)
            if (debugCalls) {
